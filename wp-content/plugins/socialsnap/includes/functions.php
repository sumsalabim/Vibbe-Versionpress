...
/**
 * Enqueue backend dashicon
 *
 * @since 1.0.0
 */
function socialsnap_enqueue_dashicon() {

	wp_enqueue_style(
		'socialsnap-dashicon',
		SOCIALSNAP_PLUGIN_URL . 'assets/css/admin-dashicon.css',
		null,
		SOCIALSNAP_VERSION
	);
}
add_action( 'admin_enqueue_scripts', 'socialsnap_enqueue_dashicon' );
...
function socialsnap_get_share_url( $network, $args = array() ) {
	...
	if ( '' === $args['permalink'] ) {
		if ( class_exists( 'WooCommerce' ) && is_checkout() || $args['post_id'] <= 0 ) {
			$args['permalink'] = socialsnap_get_shared_permalink( array( 'permalink' => get_bloginfo('url'), 'network' => $network ) );
		} else {
			$args['permalink'] = socialsnap_get_shared_permalink( array( 'post_id' => $args['post_id'], 'network' => $network ) );
		}
	}
	...
	// Title
	if ( class_exists( 'WooCommerce' ) && is_checkout() || $args['post_id'] <= 0 ) {
		$args['title'] = socialsnap_get_shared_title( array( 'title' => get_bloginfo( 'name' ), 'network' => $network, 'location' => $args['location'] ) );
	} elseif ( '' === $args['title'] ) {
		$args['title'] = socialsnap_get_shared_title( array( 'post_id' => $args['post_id'], 'network' => $network, 'location' => $args['location'] ) );
	} else {
...
function socialsnap_get_shared_permalink( $args = array() ) {
	...
	if ( '' === $args['permalink'] ) {
		if ( '' === $args['post_id'] ) {
			$args['permalink'] = socialsnap_get_current_url();
		} else {
...
function socialsnap_get_current_page_title( $post_id = 0 ) {
	...
	if ( class_exists( 'WooCommerce' ) && is_checkout() || socialsnap_is_homepage() ) {
...
function socialsnap_settings_upgrade_button( $title = '' ) {
	...
	if ( ! empty( $title ) ) {
		echo '<i class="ss-tooltip ss-question-mark" data-title="' . esc_attr( $title ) . '"></i>';
	}

	?>

	<a href="<?php echo socialsnap_upgrade_link(); ?>" class="<?php echo esc_attr( $class ); ?>" target="_blank"<?php echo $atts; ?>>
		<?php _e( 'Upgrade', 'socialsnap' ); ?>	
	</a>
...
function socialsnap_settings_require_upgrade( $settings ) {
	...
	$return = isset( $settings['pro'] ) && $settings['pro'] && ! class_exists( 'SocialSnap_Pro' );
...
function socialsnap_signature() {
	...
	$output = '<div class="ss-powered-by">Powered by <a href="' . esc_attr( $url ) . '" target="_blank" rel="nofollow noopener"><i class="ss ss-socialsnap"></i>Social Snap</a></div><!-- END .ss-powered-by -->';

	echo apply_filters( 'socialsnap_signature', $output );
}
...
function socialsnap_get_meta_values( $key = '' ) {
	...
	$results 	= $wpdb->get_results( $wpdb->prepare( $sql, $key, 'publish' ));
	$return 	= array();
...
if ( ! function_exists( 'socialsnap_print_notice' ) ) {
	function socialsnap_print_notice( $args ) {
		...
		<div id="<?php echo $args['message_id']; ?>" class="notice socialsnap-notice notice-<?php echo $args['type']; ?><?php echo $is_dismissible; ?><?php echo $class; ?>">
			<?php printf( wp_kses( $args['message'], socialsnap_get_allowed_html_tags() ) ); ?>
		</div>

		<script type="text/javascript">
			jQuery( document ).ready( function ( $ ) {

				var msgid = "<?php echo $args['message_id']; ?>";

				$( '#' + msgid ).on( 'click', '.notice-dismiss, .socialsnap-notice-dismiss-button', function ( event ) {

					var expires = "<?php echo $args['expires']; ?>";
					var nonce = "<?php echo wp_create_nonce( 'socialsnap_dismiss_notice' ); ?>";
