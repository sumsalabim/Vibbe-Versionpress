...
class SocialSnap_Metabox {
	...
	public function render_classic( $post ) { 
		...
		foreach ( $this->metabox['options'] as $id => $option_group ) {
			...
			if ( isset( $option_group['icon'] ) ) {
				$option_group['icon'] = '<i class="'. $option_group['icon'] .'"></i> ';
			} else {
				$option_group['icon'] = '';
			}
		...
		foreach ( $this->metabox['options'] as $id => $option_group ) {
			...
			foreach ( $option_group['options'] as $option ) {
				...
				if ( $value != "" ) {
					$option['value'] = $value;
				} else if ( metadata_exists( 'post', $post->ID, $option['id'] ) ) {
					$option['value'] = "";
				} else {
		...
		echo apply_filters( 'socialsnap_metabox_output', $output );
	}
	...
	public function render_block_editor( $post ) { 
		...
		foreach ( $this->metabox['options'] as $id => $option_group ) {
			...
			if ( isset( $option_group['icon'] ) ) {
				$output .= '<i class="'. $option_group['icon'] .'"></i> ';
			}
		...
		echo apply_filters( 'socialsnap_metabox_block_editor_output', $output );
	}
}
