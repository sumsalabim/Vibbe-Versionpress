...
class SocialSnap_Addons extends SocialSnap_Admin_Page {
		...
		<div id="ss-addons" class="ss-page-wrapper ss-clearfix">
			...
			<?php if ( empty( $this->addons ) ) { ?>
				...
				<div class="error notice">
					<p><?php _e( 'There was an issue retrieving the addons for this site. Please click on the button above the refresh the addons data.', 'socialsnap' ); ?></p>
				</div>
					...
					<div class="ss-col-4">
						<div class="ss-addon">
							...
							<div class="ss-addon-description">
								<h5><?php echo $addon['name']; ?></h5>
								<p><?php echo $addon['desc']; ?></p>
							</div>

							<div class="ss-addon-footer ss-<?php echo $status; ?>">

								<?php if ( 'upgrade' != $status ) { ?>
									<div class="ss-addon-status"><?php echo $status_label; ?></div>
								<?php } ?>
								
								<span class="ss-addon-action">
									<span class="spinner"></span>
									<a href="<?php echo $button_url; ?>" class="ss-button ss-button-secondary <?php echo $button_action; ?>" <?php echo $new_tab; ?> data-action="<?php echo $button_action; ?>" data-nonce="<?php echo wp_create_nonce( 'socialsnap_addon' ); ?>" data-plugin="<?php echo $plugin_basename; ?>"<?php echo $disabled; ?>><?php echo $button_label; ?></a>
								</span>
							</div>
						</div>
					</div><!-- END .ss-col-4 -->
	 ...
	 * @since 1.0.0
	 */
	function socialsnap_addons_notice() {
