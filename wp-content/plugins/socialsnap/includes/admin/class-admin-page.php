...
class SocialSnap_Admin_Page {
	...
	public function admin_header() {
		...
		<div id="ss-header">
			<img src="<?php echo SOCIALSNAP_PLUGIN_URL . 'assets/images/socialsnap.svg'; ?>" />
		</div><!-- END #socialsnap-header -->

		<?php if ( 'socialsnap-addons' === $_GET['page'] ) { ?>
			<h1 class="heading-title ss-clearfix">
				<?php 
				_e( 'Available Addons', 'socialsnap' );
				...
				$addons_button = '';
				echo apply_filters( 'socialsnap_header_bar_button', $addons_button, 'socialsnap-addons' ); 
				?>
			</h1><!-- END .heading-title -->
		<?php } elseif ( 'socialsnap-statistics' === $_GET['page'] ) { ?>
			<h1 class="heading-title ss-clearfix">
				<?php _e( 'Statistics', 'socialsnap' ); ?>
			</h1><!-- END .heading-title -->
		<?php } elseif ( 'socialsnap-license' === $_GET['page'] ) { ?>
			<h1 class="heading-title ss-clearfix">
				<?php _e( 'License Activation', 'socialsnap' ); ?>
			</h1><!-- END .heading-title -->
		<?php }
	}
}