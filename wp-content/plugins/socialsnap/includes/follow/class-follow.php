...
class SocialSnap_Social_Follow {
	...
	public function register_shortcodes( $atts ) {
				...
				foreach ( $networks as $network ) {
					...
					if ( isset ( $this->configured_networks[ $network ] ) ) {
						...
						$a['networks'][ $network ] = array(
							...
							'label'				=> sprintf( __( 'Follow us on %1$s', 'socialsnap' ), socialsnap_get_network_name( $network ) ),
							'manual_followers'	=> '',
						);
					}
				}
		...
		if ( $a['total_followers'] || is_admin() ) { 
			...
			<h4 class="ss-follow-total-counter">
				<strong><?php echo socialsnap_format_number( $total_followers ); ?></strong> <?php echo _n( 'Follower', 'Followers', $total_followers, 'socialsnap' ); ?>
			</h4>
		<?php } ?>

		<div class="<?php echo $class; ?>">

			<?php foreach ( $a['networks'] as $network_id => $network_settings ) {
				...
				$additional_data .= ' data-ss-sf-network-id="' . $network_id . '"';
				?>

				<div class="ss-follow-column"<?php echo $additional_data; ?>>
					<a href="<?php echo $network_url; ?>" class="ss-follow-network ss-<?php echo $network_id; ?>-color" rel="nofollow noopener" aria-label="<?php echo esc_attr( $network_id ); ?>" target="_blank">
						<i class="ss ss-<?php echo $network_id; ?>"></i>

						<?php if ( $a['labels'] && $network_settings['label'] || is_admin() ) { ?>
							<span class="ss-follow-network-label"><?php echo $network_settings['label']; ?></span>
						<?php } ?>

						<?php if ( $a['button_followers'] || is_admin() ) {

							$count = (int) $this->get_follow_count( $network_id ); ?>

							<span class="ss-follow-network-count">
								<span class="ss-follow-network-count-number"><?php echo socialsnap_format_number( $count ); ?></span>
								<span class="ss-follow-network-count-label"><?php echo _n( 'Follower', 'Followers', $count, 'socialsnap' ); ?></span>
							</span>
	...
	private function get_follow_count( $network = '' ) {
		...
		if ( in_array( $network, $api_networks, true ) && in_array( $network, array_keys( $this->authorized_networks ), true ) || in_array( $network, $this->automatic_networks, true ) ) {
			...
			if ( false === $cache ) {
				...
				if ( $c = get_option( 'socialsnap_follow_count_' . $network . '_' . $network_settings['profile']['username'] ) ) {
					$cache = $c;
				}
		...
		<div class="ss-preview-screen ss-preview-social_follow">
			<div class="ss-follow-preview-shortcode">
				...
				<div class="ss-follow-configure-note">
					<?php _e( 'Please configure at least one network to show a preview.', 'socialsnap' ); ?>
				</div>

				<?php echo do_shortcode( '[ss_social_follow]' ); ?>
			</div>
		</div>
		<?php
	}
	...
	public function block_editor_support() {
		...
		register_block_type( 'socialsnap/social-follow', array(
	...
	public function expired_counts() {
		...
		// $expired_api_counts 		= array_intersect( $this->expired_counts, socialsnap_social_follow_networks_with_api() );
		// $expired_automatic_counts 	= array_intersect( $this->expired_counts, socialsnap_social_follow_networks_automatic() );
